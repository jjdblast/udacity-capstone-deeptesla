...
def get_human_steering(epoch_id):
    ...
    steering_path = utils.join_dir(epoch_dir, 'epoch{:0>2}_steering.csv'.format(epoch_id))
    assert os.path.isfile(steering_path)
    
    rows = utils.fetch_csv_data(steering_path)
...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    if perform_smoothing:
        machine_steering = list(utils.smooth(np.array(machine_steering)))
        #human_steering = list(utils.smooth(np.array(human_steering)))
    ...
    front_vid_path = utils.join_dir(epoch_dir, 'epoch{:0>2}_front.mkv'.format(epoch_id))
    assert os.path.isfile(front_vid_path)
    
    dash_vid_path = utils.join_dir(epoch_dir, 'epoch{:0>2}_dash.mkv'.format(epoch_id))
    ...
    vid_size = utils.video_resolution_to_size('720p', width_first=True)
    out_path = utils.join_dir(out_dir, 'epoch{:0>2}_human_machine.mkv'.format(epoch_id))
    ...
    for f_cur in range(len(machine_steering)):
        ...
        ry0, rh = 80, 500
        dimg = dimg[100:, :930]
        dimg = utils.cv2_resize_by_height(dimg, h-rh)
        ...
        wimg = utils.imread(os.path.abspath("images/wheel-tesla-image-150.png"), cv2.IMREAD_UNCHANGED)

        human_wimg = utils.rotate_image(wimg, -human_steering[f_cur])
        wh, ww = human_wimg.shape[:2]
        fimg = utils.overlay_image(fimg, human_wimg, y_offset = rh+50, x_offset = dw+60)

        ####################### machine steering wheels ######################
        disagreement = abs(machine_steering[f_cur] - human_steering[f_cur])
        machine_wimg = utils.rotate_image(wimg, -machine_steering[f_cur])
        ...
        fimg = utils.overlay_image(fimg, machine_wimg, y_offset = rh+50, x_offset = dw+260)
        
        ####################### text ######################
        timg_green_agree = utils.imread(os.path.abspath("images/text-green-agree.png"), cv2.IMREAD_UNCHANGED)
        timg_ground_truth = utils.imread(os.path.abspath("images/text-ground-truth.png"), cv2.IMREAD_UNCHANGED)
        timg_learned_control = utils.imread(os.path.abspath("images/text-learned-control.png"), cv2.IMREAD_UNCHANGED)
        timg_red_disagree = utils.imread(os.path.abspath("images/text-red-disagree.png"), cv2.IMREAD_UNCHANGED)
        timg_tesla_control_autopilot = utils.imread(os.path.abspath("images/text-tesla-control-autopilot.png"), cv2.IMREAD_UNCHANGED)
        timg_tesla_control_human = utils.imread(os.path.abspath("images/text-tesla-control-human.png"), cv2.IMREAD_UNCHANGED)
        # timg_ = utils.imread(os.path.abspath("images/text-.png"), cv2.IMREAD_UNCHANGED)

        fimg = utils.overlay_image(fimg, timg_tesla_control_autopilot, y_offset = rh+8, x_offset = dw+83)
        fimg = utils.overlay_image(fimg, timg_learned_control, y_offset = rh+8, x_offset = dw+256)
        fimg = utils.overlay_image(fimg, timg_ground_truth, y_offset = rh+205, x_offset = dw+90)
        fimg = utils.overlay_image(fimg, timg_red_disagree, y_offset = rh+205, x_offset = dw+230)
        fimg = utils.overlay_image(fimg, timg_green_agree, y_offset = rh+205, x_offset = dw+345)
    ...
    utils.mkv_to_mp4(out_path, remove_mkv=True)
